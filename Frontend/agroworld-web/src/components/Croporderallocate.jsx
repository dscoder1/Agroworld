...
const Croporderallocate = () => {
  ...
  const fun = async () => {
    ...
    const response = await axios.get(`http://localhost:3000/allocatecroporder/${id}`)
    console.log(response.data)
    console.log(response.data[0].Vegetablename)
    setvegetablename(response.data[0].Cropname)
  ...
  const handleCropAllocateForm = async (e) => {
    e.preventDefault();
    ...
    const response = await axios.post("http://localhost:3000/allocatecroporder", { vegetablekg, vegetablename, vegetableprice, vegetablesellernumber, locationorder, vegetableallocaternumber, vegetablebuyernumber,iddata,logindata })
    console.log(response.data)
  ...
  return (
    <>
      ...
      <div className="rightSignup">
        <form method='post' onSubmit={handleCropAllocateForm} id='specialform'>
...
export default Croporderallocate
