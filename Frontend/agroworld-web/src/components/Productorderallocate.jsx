...
const Productorderallocate = () => {
  ...
  const fun = async () => {
    ...
    const response = await axios.get(`http://localhost:3000/allocateproductorder/${id}`)
...
setlogindata(response.data[0].LoginUser)
    ...
    setdescdata(response.data[0].Desc)

  }
  const handleProductAllocateForm = async (e) => {
    e.preventDefault();
    console.log(vegetablekg, vegetablename, vegetableprice,  locationorder, vegetableallocaternumber, vegetablebuyernumber);
    const response = await axios.post("http://localhost:3000/allocateproductorder", { vegetablekg, vegetablename, vegetableprice,locationorder, vegetableallocaternumber, vegetablebuyernumber,descdata ,iddata,logindata})
    console.log(response.data)
  ...
  return (
     ...
     <Adminnavbar/>
      ...
      <div className="rightSignup">
        <form method='post' onSubmit={handleProductAllocateForm} id='specialform'>
          ...
          <input type="text" placeholder='Price Per Kg' id='passSignup' value={descdata} />
...
export default Productorderallocate
